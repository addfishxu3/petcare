...
#define FILE_PHOTO "/S__60252174.jpg"
int sendinterval=10;
...
void setup() {
  ...
  if(!Firebase.RTDB.beginStream(&fbdo1, "/User/"+uid+"/petList/"+USER_PET+"/Cam/intervalsec")){
...
void loop() {
  ...
  if (Firebase.ready()){
    ...
    if(fbdo1.streamAvailable()){
      if(fbdo1.dataType() == "int"){
        if(Firebase.RTDB.getInt(&fbdo1, "/User/"+uid+"/petList/"+USER_PET+"/Cam/intervalsec")){
          sendinterval=fbdo1.intData(); 
        }
      }
    }
  }
  ...
  String t="pic/"+uid+"/";
    if (Firebase.Storage.upload(&fbdo, STORAGE_BUCKET_ID /* Firebase Storage bucket id */, FILE_PHOTO /* path to local file */, mem_storage_type_flash /* memory storage type, mem_storage_type_flash and mem_storage_type_sd */, t+USER_PET+FILE_PHOTO /* path of remote file stored in the bucket */, "image/jpeg" /* mime type */)){
